...
class Game extends React.Component {
	...
	noMoveToDuplicateSelectGrid(selectedGrids, nowGrids) {
		console.log("nowGrids");
		console.log(nowGrids);
		console.log("before");
		console.log(selectedGrids);
		for (let i = 0; i < selectedGrids.length; i++) {
			let grid = selectedGrids[i];
			for (let j = i + 1; j < selectedGrids.length; j++) {
				if (grid[0] === selectedGrids[j][0] && grid[1] === selectedGrids[j][1]) {
					selectedGrids[i] = nowGrids[i];
					selectedGrids[j] = nowGrids[j];
				}
			}
		}
		console.log("after");
	...
	handleClickSquare(event) {
		...
		if (this.state.clickedGridCache.length === 4) {
			...
			console.log("hogehogeopjfeowe");
			console.log(stateCopy.playerStayGridNow);
			stateCopy.clickedGridCache = this.noMoveToDuplicateSelectGrid(stateCopy.clickedGridCache, stateCopy.playerStayGridNow);
			for (let i = 0; i < 4; i++) {
				let nextGrid = stateCopy.clickedGridCache[i];
				stateCopy.stateOfGameBoard[stateCopy.countTurn + 1][nextGrid[0]][nextGrid[1]] = {player: (i === 0 || i === 1) ? 1 : 2};
			}
			stateCopy.countTurn += 1;
			console.log("clickdGridCache");
			console.log(stateCopy.clickedGridCache);
			stateCopy.playerStayGridNow = Object.assign([], stateCopy.clickedGridCache);
			console.log("new playerStayGridNow");
			console.log(stateCopy.playerStayGridNow);
